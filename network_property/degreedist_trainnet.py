...
def plot_d_dist(graphs,labels):
    ...
    #plt.show()
def plot_from_file():
    pkid = np.loadtxt('pkid.csv')
    pk = np.loadtxt('pk.csv')
    ...
    plt.plot(xs,ys,'-',label='Univariate spline interpolation',linewidth=2,color='blue',alpha=0.86)
    plt.grid()
    plt.legend(loc="best")
    plt.xlabel('$Degree(\log(k))$')
    plt.ylabel('$Probability(\log(p_k))$')
    plt.title('Degree distribution for train set of 10 networks')
    plt.savefig('deg_dist_train')
    plt.show()
if __name__ == "__main__":
   ...
   # plot_d_dist(g)
    ...
    # np.savetxt('pkid.csv', np.concatenate(pkid_arr), delimiter=',', fmt='%s')
    # np.savetxt('pk.csv', np.concatenate(pk_arr), delimiter=',', fmt='%s')
